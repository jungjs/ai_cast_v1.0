...
- **Request Parameters**:
    - `file` (File, Required): JPG 이미지 파일 (20MB 이내, F-27)
    - `target_langs` (String, Required): 번역 대상 언어 코드
- **Response (200 OK)**:
  ```json
  {
...
|:---:|:---|:---|
| 200 | OK | 요청 성공 |
| 400 | Bad Request | 필수 파라미터 누락 또는 잘못된 요청 (파일 포맷 불일치 등) |
| 401 | Unauthorized | API 키 인증 실패 (키 누락, 미등록, is_active=0) |
| 403 | Forbidden | 권한 부족 (관리자 전용 API 접근 시) |
| 404 | Not Found | 요청한 리소스를 찾을 수 없음 |
| 413 | Payload Too Large | 파일 크기 초과 (이미지 20MB 제한 등) |
...
---
**최종 업데이트**: 2026-05-12
**특이사항**: 요구사항 재번호(F-01~F-36) 반영, 인증 API 제거(Filter 처리), AI 호출 통계 API(일/주/월) 신규, 리소스 모니터링·API 사용현황 API 재구성, 이미지 파이프라인(OCR) 응답 구조 추가, DB 약어 컬럼명 반영, 상태 코드 413/429/503 추가
